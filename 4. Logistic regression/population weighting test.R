...
popweighting <- "Population-weighting-test.xlsx"
popweighting <- readxl::read_excel(popweighting)
popweighting$COUNTRY<-popweighting$Country
# Performing left join
merged_df <- merge(finaldatasets, popweighting, by = "COUNTRY", all.x = TRUE)
finaldatasets <- merged_df
print(merged_df)
...
merged_data <-merged_data [ , c("COUNTRY.x","code","NAME_2.x",
                                "rainfallzone","Humidityzone", "aerozone","windspeedzone",
                                "Pp_dnst", "nw_lnd_", "epidemic", "district_country","geometry.y","Population density", "Fertility rate",         
...
merged_data <-merged_data [ , c("epidemic", "rainfall3"  ,"Aero3",  "Pp_dnst",   
                                "humidity3", "windspeed3", "Land_category","Population density", "Fertility rate",         
                                "Population growth rate" )]
...
# Fit logistic regression model with optimal weight
...
# VIF values of each variable in the model
car::vif(model2)
...
plot(roc_curve, main = "Receiver Operating Characteristic (ROC) Curve",
     ...
     auc.polygon.col = colors[2], grid = TRUE, grid.col = "lightgray", grid.lty = 3,
...
perform_analysis <- function(data) {
  ...
  backwardsmodel <- glm(epidemic ~ Aero3 + humidity3 + rainfall3 + Land_category, 
                        data = merged_data, 
                        family = "binomial",
                        weights = ifelse(merged_data$epidemic == 1, 0.805, 0.195),
                        method = "brglmFit")
...
merged_data$weights <- ifelse(merged_data$epidemic == 1, 0.805, 0.195)
merged_data$popdensityweights<- merged_data$weights * merged_data$"Population density"
merged_data$fertilityweights<- merged_data$weights * merged_data$"Fertility rate"
merged_data$popgrowthrate<- merged_data$weights * merged_data$`Population growth rate`
...
model2 <- glm(epidemic ~ Aero3 + humidity3 + rainfall3+ Land_category, 
              data = merged_data, 
              family = "binomial",
              weights = merged_data$popdensityweights,method = "brglmFit")
# Summarize model
summary(model2)
...
model2 <- glm(epidemic ~ Aero3 + humidity3 + rainfall3+ Land_category, 
              data = merged_data, 
              family = "binomial",
              weights = merged_data$fertilityweights,method = "brglmFit")
# Summarize model
summary(model2)
...
model2 <- glm(epidemic ~ Aero3 + humidity3 + rainfall3+ Land_category, 
              data = merged_data, 
              family = "binomial",
              weights = merged_data$popgrowthrate,method = "brglmFit")
# Summarize model
summary(model2)
...
merged_data$Pp_dnstweight<- merged_data$weights * merged_data$Pp_dnst

# Fit logistic regression model with optimal weight
model2 <- glm(epidemic ~ Aero3 + humidity3 + rainfall3+ Land_category, 
              data = merged_data, 
              family = "binomial",
              weights = merged_data$Pp_dnst,method = "brglmFit")
# Summarize model
summary(model2)
