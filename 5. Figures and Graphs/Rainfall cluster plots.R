...
layer_names <- names(r_resampled)

# Extract month from: chirps.v2.0_2002.01
months <- sub("^.*_[0-9]{4}\\.([0-9]{2})$", "\\1", layer_names)

# Split raster stack by month
...
# OPTIONAL: create separate objects Jan, Feb, ..., Dec
...
cl2test<-rainfallcat [ , c('GID_2', 'NAME_2_x', 'zonalcat')]
...
compare <- data_long %>%
  ...
  scale_y_continuous(
    breaks = seq(40, 135, by = 5),
    limits = c(40, 135),        # ensures axis is drawn
    expand = c(0.02, 0.02)
  ) +
  theme(
...
rainfallcat $district_country <- paste(rainfallcat $NAME_2_x, rainfallcat $COUNTRY_x, sep = " ")
#create district intersection for weighting later

plot(rainfallcat ['zonalcat'])
rainfallcat $zonal_cat_numeric <- ifelse(rainfallcat $zonalcat == "Class 1", as.numeric("1"),
                                         ifelse(rainfallcat $zonalcat == "Class 2", as.numeric("2"),
                                                ifelse(rainfallcat $zonalcat == "Class 3", as.numeric("3"),
                                                       ifelse(rainfallcat $zonalcat == "Class 4", as.numeric("4"),
                                                              ifelse(rainfallcat $zonalcat == "Class 5", as.numeric("5"),
                                                                     ifelse(rainfallcat $zonalcat == "Class 6", as.numeric("6"), NA))))))

column_to_raster <- rainfallcat $zonal_cat_numeric

# Create a raster template from the shapefile
raster_template <- raster(extent(rainfallcat ), res = 0.1)  # You can adjust resolution as needed

# Convert the column to raster
rasterized_column <- rasterize(rainfallcat , raster_template, field = column_to_raster)
