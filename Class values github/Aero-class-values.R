...
for (i in 1:nrow(cl2test)) {
  if (is.na(cl2test$zonalcat[i])) {
    mode_value <- with(cl2test, zonalcat[COUNTRY == COUNTRY[i] & NAME_1 == NAME_1[i]])
    cl2test$zonalcat[i] <- ifelse(length(mode_value) > 0, get_mode(mode_value), NA)
  }
}


# Function to find mode
get_mod2e <- function(x) {
  ux <- unique(x)
  ux[which.max(tabulate(match(x, ux)))]
}
...
for (i in 1:nrow(cl2test)) {
  if (is.na(cl2test$zonalcat[i])) {
    mode_value <- with(cl2test, zonalcat[COUNTRY == COUNTRY[i]])
    cl2test$zonalcat[i] <- ifelse(length(mode_value) > 0, get_mode(mode_value), NA)
  }
} 
rows_with_missing <- cl2test[is.na(cl2test$zonalcat), ]
...
#st_write(total3test, "Aerotestreducedclasses.shp",append=FALSE)
#st_write(total3test, "Aerotestreducedclassesfilledin.shp",append=FALSE)
